...
//					doSSLClose(key);
	 ...
	 * buf.put("HTTP/1.1 200 OK \r\n".getBytes()); buf.flip(); SocketChannel sc
	 * = (SocketChannel) key.channel(); while (buf.hasRemaining()) {
	 * System.out.println("server response ..."); sc.write(buf); }
	 * buf.compact();
	 * 
	 * }
	 */
	...
	private void doShakeHand(SocketChannel sc) throws IOException {
		...
		while (!handShakeDone) {
			...
			case NOT_HANDSHAKING:
				...
				sc.register(selector, SelectionKey.OP_READ);// register the read
															// and write
				// event
				handShakeDone = true;
				break;
